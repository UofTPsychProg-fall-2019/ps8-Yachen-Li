...
tbl_clean <- select(tbl, session_id:labels)
...
tbl_clean <- filter(tbl, !is.na(D_biep.White_Good_all))
...
tbl_clean$gender4 <- recode(tbl_clean$gender, "[2]" = 'Female', "[1]" = 'Male', "[5]" = 'Queer', .default = "Other")
...
# white men
white_biasM <- filter(tbl_clean, race == 6 & gender4 == 'Male')
mean(white_biasM$bias)

# white women

white_biasW <- filter(tbl_clean, race == 6 & gender4 == 'Female')
mean(white_biasW$bias)

# advanced degree holders who are men
advance_men <- filter(tbl_clean, edu7 == "advanced degree" & gender4 == 'Male')
mean(advance_men$bias)

# high school graduates who are men
highschool_men <- filter(tbl_clean, edu7 == "highschool graduate" & gender4 == 'Male')
mean(highschool_men$bias)
